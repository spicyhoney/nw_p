...
> **歷史混合文件（2026-07-08）**：本頁同時包含早期檢查與尚未實作的資料構想，
> 其中部分推論已被後續稽核修正。現在的資料事實以
> [品質報告](../reports/data_quality.md)、[資料政策](data-policy.md)與
> [資料字典](data-dictionary.md)為準；不得從本頁推論功能已完成。
...
| `order_record範例資料.json/.csv` | JSON 與 CSV 都是 99 筆範例訂單 | ✅ 已由品質報告複核 |
...
1. **order_type=07、service_id=18、vendor_id=15 未在任何 schema 註解與主檔中定義**（64/99 筆）——不得猜成正式服務；目前保留原值並隔離，不供 Agent 使用。Demo 使用的 synthetic／curated 資料必須另帶來源標籤。
...
6. 兩份訂單範例經目前 pipeline 複核都是 99 筆；CSV 與 JSON 的差異應以
   品質報告中的欄位與 shape 檢查為準。
...
- `mms_order_record`：訂位／勘驗訂單的官方範例結構；目前共有 99 筆範例，
  不足以推論正式營運比率
...
| 訂單歷史 | 早期構想：以 99 筆官方範例為參考，另產生有來源標籤的 synthetic 歷史 | 待決定 | 不得把 synthetic 與官方範例合併後宣稱為真實營運資料 |
...
1. **需求理解**：建立固定口語案例與預期輸出，再報告實際通過率；目前尚未
   完成固定 HF eval，不能預先宣稱特定準確率或 F1。
2. **服務媒合**：媒合計分公式攤開講（地區/專長/評分/時段加權），附一筆案例的計分明細表——透明可解釋，反而比黑盒 ML 加分
3. **商業洞察**：99 筆品質不完整的範例訂單只能用來說明 schema 與資料缺口，
   不能把其狀態比例當成正式退款率或因果證據。
...
- **目前實作邊界**：只使用 synthetic contact，pending 遮罩、accepted 才揭露；
  真實 PII 的 AES-256-GCM、hash 查詢、金鑰管理、正式登入與刪除政策都尚未完成。
- **簡報呈現**：把 synthetic 遮罩與狀態授權列為已完成；加密、KMS、
  Cognito／SSO、mTLS 與正式告警只能列在未來規劃。
