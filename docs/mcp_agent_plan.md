...
> **歷史規劃文件（2026-07-26）**：本頁主要是早期完整功能規劃，不代表所有 Tool
> 已完成。實際程式狀態以 [實作索引](implementation-index.md) 與
> [MCP Server README](../src/home_repair_agent/mcp_server/README.md) 為準；未完成
> 工作只以 [TASKS](../TASKS.md) 為準。
> 目前已完成 `search_services`、`resolve_location`、`get_consultation_form`、
> `match_service_providers` 四個唯讀 Tool，以及可替換 ModelClient 的本機 Agent
> 迴圈。真實 Bedrock adapter、寫入與下方其他候選 Tool 尚未完成，名稱與 schema
> 在實作前仍可調整。
