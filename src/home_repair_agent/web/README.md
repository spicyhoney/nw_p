...
這個模組把本機 Agent、四個唯讀 MCP Tools、synthetic Demo repository 與受控
...
- 案件、訂單、idempotency 與 audit 尚未寫進 PostgreSQL。
...
- `DemoCaseWorkflowRepository`：P0 process-local 儲存。
...
- session、案件、訂單與 audit 只存在目前 Python process；重啟就消失。
...
- 此模組不含 SQL、正式資料庫憑證、AWS 金鑰或寫入 MCP Tool。
